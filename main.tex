...
{\begin{itemize}
  \item Researched and implemented novel data formats and quantization algorithms for deep learning using specialized hardware.
        Ran hundreds of experiments on quantized LLM training and inference.
  \item Engineering lead and codebase owner for our team's LLM training infrastructure and quantization library.
  ...
  \item Work had major impact on the Azure Maia 100 accelerator and OpenAI inference workloads. Research led to three publications [1,2,3] and four successful patents.
\end{itemize}
}
}
...
{\begin{itemize}
  ...
  \item Proposed and developed new methods to automate the quantization flow of trained models.
\end{itemize}
}
}
...
{\begin{itemize}
  \item Assisted the research and development of a Matlab-like programming language which statically compiles to optimized code for CPU or GPU.
  \item Performed extensive engineering on the GPU back-end using CUDA. Obtained 16x speedup on option pricing with Monte Carlo Black-Scholes compared to hand-optimized C.
\end{itemize}
}
}
